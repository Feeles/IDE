;!function () {
        ...
        case 'capture':
          ...
          if (canvas) {
            ...
            const promise = e.data.type === 'image/jpeg'
              ? domtoimage.toJpeg(document.body, {bgcolor: bgcolor})
  ...
  feeles.fetch = function (name) {
    return requestPostMessage('fetch', name, function (event, resolve, reject) {
      if (event.data.error) {
        reject('Error in feeles.fetch in ' + name);
      } else {
  ...
  feeles.resolve = function (name) {
    return requestPostMessage('resolve', name, function (event, resolve, reject) {
      if (event.data.error) {
        reject('Error in feeles.resolve in ' + name);
      } else {
  ...
  if (SpeechRecognition) {
    feeles.SpeechRecognition = class PseudoSpeechRecognition extends SpeechRecognition {
      start() {
        ...
        requestPostMessage('api.SpeechRecognition', self, function (e) {
